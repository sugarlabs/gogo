...
class Serial(SerialBase):
    ...
    def open(self):
        ...
        if self._port is None:
            raise SerialException(_("Port must be configured before it can be used."))
        self.fd = None
        #open
        try:
    ...
    def _reconfigurePort(self):
        ...
        if self.fd is None:
            raise SerialException(_("Can only operate on a valid port handle"))
        ...
        try:
            ...
            try:
                ...
                raise ValueError(_('Invalid baud rate: %r') % self._baudrate)
        #setup char len
        cflag &= ~TERMIOS.CSIZE
        if self._bytesize == 8:
            cflag |= TERMIOS.CS8
            ...
            raise ValueError(_('Invalid char len: %r') % self._bytesize)
        #setup stopbits
        if self._stopbits == STOPBITS_ONE:
            ...
            raise ValueError(_('Invalid stopit specification: %r') % self._stopbits)
        #setup parity
        ...
        if self._parity == PARITY_NONE:
            ...
            raise ValueError(_('Invalid parity: %r') % self._parity)
        #setup flow control
        #xonxoff
        ...
        if vmin < 0 or vmin > 255:
            raise ValueError(_('Invalid vmin: %r ') % vmin)
        cc[TERMIOS.VMIN] = vmin
        #vtime
        if vtime < 0 or vtime > 255:
            raise ValueError(_('Invalid vtime: %r') % vtime)
    ...
    def write(self, data):
        ...
        if not isinstance(data, str):
            raise TypeError(_('expected str, got %s') % type(data))
        t = len(data)
        d = data
        while t > 0:
            try:
