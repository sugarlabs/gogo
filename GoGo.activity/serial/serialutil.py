...
class FileLike(object):
    ...
    def readlines(self, sizehint=None, eol='\n'):
        ...
        if self.timeout is None:
            raise ValueError, _("Serial port MUST have enabled timeout for this function!")
        lines = []
        while 1:
...
class SerialBase(FileLike):
    ...
    def setBaudrate(self, baudrate):
        ...
        try:
            ...
            raise ValueError(_("Not a valid baudrate: %r") % (baudrate,))
        else:
    ...
    def setByteSize(self, bytesize):
        """Change byte size."""
        if bytesize not in self.BYTESIZES: raise ValueError(_("Not a valid byte size: %r") % (bytesize,))
    ...
    def setParity(self, parity):
        ...
        if parity not in self.PARITIES: raise ValueError(_("Not a valid parity: %r") % (parity,))
        self._parity = parity
        if self._isOpen: self._reconfigurePort()
    ...
    def setStopbits(self, stopbits):
        ...
        if stopbits not in self.STOPBITS: raise ValueError(_("Not a valid stopbit size: %r") % (stopbits,))
    ...
    def setTimeout(self, timeout):
        ...
        if timeout is not None:
            if timeout < 0: raise ValueError(_("Not a valid timeout: %r") % (timeout,))
            try:
                timeout + 1     #test if it's a number, will throw a TypeError if not...
            except TypeError:
                raise ValueError(_("Not a valid timeout: %r") % (timeout,))
